...
export default async(req)=>{
 ...
 const by={};for(const wanted of t.sizes){const v=enabled.find(v=>{const title=norm(v.title);const colorOK=!t.color||title.includes(norm(t.color));return colorOK&&sizeOf(v.title)===wanted;});by[wanted]=v?{id:v.id,title:v.title,sku:v.sku,cost:v.cost}:null;}const missing=Object.keys(by).filter(s=>!by[s]);return{website_name:t.website_name,status:missing.length?'variant_review_needed':'ready',printify_product_id:p.id,printify_title:p.title,missing_sizes:missing,variants_by_size:by,all_enabled_variants:enabled.map(v=>({id:v.id,title:v.title,is_available:v.is_available})),candidate_count:cs.length};});return Response.json({ok:true,mode:'mens_variant_audit',total:rows.length,ready:rows.filter(x=>x.status==='ready').length,review_needed:rows.filter(x=>x.status!=='ready').length,rows});}catch(e){return Response.json({ok:false,error:'Unable to audit mens variants.'},{status:502});}
};
